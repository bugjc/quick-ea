...
public class CertUtil {
	...
	private static void init() {
		try {
			...
			addProvider();
			//初始化签名私钥证书
			initSignCert(qrCodeConfig);
			//初始化验签证书的中级证书
			//initMiddleCert();
			//初始化验签证书的根证书
			//initRootCert();
			//初始化加密公钥
			initEncryptCert(qrCodeConfig);
			//构建磁道加密公钥
			//initTrackKey();
			//初始化所有的验签证书
	...
	private static void initSignCert(QrCodeConfig qrCodeConfig) {
		...
		if (qrCodeConfig.getSignCertPath() == null
				|| qrCodeConfig.getSignCertPwd() == null
				|| qrCodeConfig.getSignCertType() == null) {
			log.info("WARN: " + qrCodeConfig.getSignCertPath()+ "或" + qrCodeConfig.getSignCertPwd()
					+ "或" + qrCodeConfig.getSignCertType() + "为空。 停止加载签名证书。");
			return;
		}
		if (null != keyStore) {
			keyStore = null;
		...
		try {
			keyStore = getKeyInfo(qrCodeConfig.getSignCertPath(), qrCodeConfig.getSignCertPwd(), qrCodeConfig.getSignCertType());
...
//		}
	...
	private static void initValidateCertFromDir(QrCodeConfig qrCodeConfig)  {
		...
		certMap.clear();
		String dir = qrCodeConfig.getValidateCertDir();
		...
		File fileDir = null;
		try {
			fileDir = ResourceUtils.getFile(dir);
		} catch (FileNotFoundException e) {
			log.info("加载验证签名证书目录失败");
		}
		assert fileDir != null;
	...
	private static KeyStore getKeyInfo(String pfxkeyfile, String keypwd,
		...
		try {
			...
			File file = ResourceUtils.getFile(pfxkeyfile);
...
//				initMiddleCert();
//			} else {
//				log.info(SdkConfig.SDK_MIDDLECERT_PATH + " not set in " + SdkConfig.FILE_NAME);
//				return null;
//			}
//		}
		return middleCert;
...
//				initRootCert();
//			} else {
//				log.info(SdkConfig.SDK_ROOTCERT_PATH + " not set in " + SdkConfig.FILE_NAME);
//				return null;
//			}
//		}
		return rootCert;
