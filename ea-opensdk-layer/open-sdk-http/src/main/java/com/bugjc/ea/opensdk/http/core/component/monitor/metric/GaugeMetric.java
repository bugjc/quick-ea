 ...
 * @date 2019/12/19
 * **/
public class GaugeMetric implements Metric<Gauge, MetricGaugeIndex.MetricGauge> {
    ...
    public GaugeMetric(MetricGaugeIndex metricGauge, MetricRegistry registry){
        ...
        //预加载度量实例
        for (MetricGaugeIndex.MetricGauge metric : metricGauge.getList()) {
            this.init(metric);
        }
    }

    @Override
    public void init(MetricGaugeIndex.MetricGauge enumName) {
    ...
    @Override
    public Gauge get(MetricGaugeIndex.MetricGauge enumName) {
        for (;;) {
