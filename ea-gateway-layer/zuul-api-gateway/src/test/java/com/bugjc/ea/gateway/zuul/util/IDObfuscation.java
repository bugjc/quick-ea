...
public class IDObfuscation {
    ...
    private static int mod(int i, StringBuilder sb) {
        sb.append(ALPHABET[i % BASE]);
        return i / BASE;
    }

    private static int encode(int paddingStartIndex, int paddingNum) {
        int n = 0;
    ...
    private static int encode(byte[] bytes, int start, int end, int paddingStartIndex, int paddingNum) {
        return encode(bytes, start, end) + encode(paddingStartIndex, paddingNum);
    }

    private static int encode(byte[] bytes, int start, int end) {
        int n = 0;
    ...
    private static int base(String s) {
        int n = 0;
        int power = 1;
