-- 创建数据库和账户
CREATE DATABASE IF NOT EXISTS ea_api_gateway_server_test COLLATE = 'utf8_general_ci' CHARACTER SET = 'utf8';
GRANT ALL ON ea_api_gateway_server_test.* TO 'gateway'@'%' IDENTIFIED BY 'gateway2018';
GRANT ALL ON ea_api_gateway_server_test.* TO 'gateway'@'localhost' IDENTIFIED BY 'gateway2018';

-- 创建数据库表及插入初始示例数据
CREATE DATABASE /*!32312 IF NOT EXISTS*/`ea_api_gateway_server_test` /*!40100 DEFAULT CHARACTER SET utf8 */;

USE `ea_api_gateway_server_test`;
...
CREATE TABLE `app_route` (
  ...
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='应用路由表';
...
CREATE TABLE `app_security_config` (
  ...
  KEY `exclude_path` (`exclude_path`),
  KEY `is_signature` (`is_signature`),
  KEY `is_verify_token` (`is_verify_token`)
) ENGINE=InnoDB AUTO_INCREMENT=134 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='配置不需要安全认证的API表';
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='应用版本映射表';
